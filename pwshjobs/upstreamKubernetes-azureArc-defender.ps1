...
Write-Output ""
Write-Output "The Defender for Containers extension requires the Defender for Containers plan enabled on the subscription."
Write-Output "To be able to proceed, we ask to confirm that the Defender for Containers plan is enabled on your subscription."
Write-Output "Ref: https://learn.microsoft.com/en-us/azure/defender-for-cloud/tutorial-enable-containers-arc#enable-the-defender-for-containers-plan"
Write-Output "If the answer is no, onboarding to Azure Arc will be without Defender extension install."
Write-Output ""
$userinput = Read-Host -Prompt "Do you confirm having Defender for Containers plan enabled on your subscription? (y/n)"
if ($userinput -eq "y"){az vm run-command create --resource-group $RG --async-execution false --run-as-user $admin --script "sudo wget -O - https://raw.githubusercontent.com/marianleica/azrez/refs/heads/progress/pwshjobs/azvm-onboardingArc-defender-runcommand.sh | bash -s -- $clientId $clientSecret $tenantId" --timeout-in-seconds 3600 --run-command-name "OnboardingToArc" --vm-name kube-master-1}
else {az vm run-command create --resource-group $RG --async-execution false --run-as-user $admin --script "sudo wget -O - https://raw.githubusercontent.com/marianleica/azrez/refs/heads/progress/pwshjobs/azvm-onboardingArc-runcommand.sh | bash -s -- $clientId $clientSecret $tenantId" --timeout-in-seconds 3600 --run-command-name "OnboardingToArc" --vm-name kube-master-1
}
